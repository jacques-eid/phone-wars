...
func handle_capture(top_result: AIActionResult) -> void:
	var ai_controller: AIController = agent as AIController
	var target_cell: Vector2i = top_result.target_building.cell_pos

	if top_result.unit.cell_pos == target_cell:
...
func handle_merge(top_result: AIActionResult) -> void:
	var ai_controller: AIController = agent as AIController
	var target_cell: Vector2i = top_result.target_unit.cell_pos
