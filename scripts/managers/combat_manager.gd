...
static func resolve_combat(attacker: Unit, defender: Unit, terrain_defense: float) -> CombatResult:
	var result: CombatResult = CombatResult.new()
	result.attacker = attacker
	result.defender = defender
	result.damage = compute_damage(attacker, defender, terrain_defense)
	result.defender_killed = defender.actual_health - result.damage <= 0

	return result


static func compute_damage(attacker: Unit, defender: Unit, terrain_defense: float) -> float:
